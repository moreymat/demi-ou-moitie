...
def split_arrete(pdf, setting):
    ...
    for page in pdf.pages:
        ...
        if page_has_column(crop):  # Si la page est sous un format valide
            table = crop.extract_table(
                setting
            )  # Sépare le texte selon le setting en paramètre
            array = np.array(table)
            for column in range(2):  # Pour chacune des deux colonnes
                for line in array[:, column]:  # Pour chaque ligne
                    if len(line) > 0:  # Si ce n'est pas une ligne vide

                        temp = np.array2string(
                            line
                        )  # Renvoie un string de la forme <'string'>
                        string = temp[
                            1 : len(temp) - 1
                        ]  # Retire les <'> pour obtenir <string>

                        if (
                            (previous_line_empty and string.startswith("N° "))
                            or re.match(regex, string)
                            or re.match(regex1, string)
                        ):  # Detecte les arretés
                            result += "\n\n\n\n"
                        else:
                            if (
                                previous_line_empty
                                and string.lower().startswith("article ")
                            ) or re.match(
                                regex2, string.lower()
                            ):  # Detecte les différentes parties de l'arreté
                                result += "\n"
    ...
    output = io.open(
        r"data\text\out.txt", "w", encoding="utf-8"
...
# Génération des lignes pour séparer le texte horizontalement
ys = []
...
setting = {
    ...
    "explicit_vertical_lines": [
        ...
        height,
        width - 30,
    ],  # Ligne explicit: A gauche, au mileu, a droite pour séparer les colonnes
