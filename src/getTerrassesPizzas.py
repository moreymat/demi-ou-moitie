...
for i in range(len(data['data'])):
  for j in range(len(data['data'][i]['content']['arretes'])):
    for k in range(len(data['data'][i]['content']['arretes'][j]['title'])):
      dejaTerrasse = 0
      dejaPizza = 0
      if contains(data['data'][i]['content']['arretes'][j]['title'][k]['text'], "- Terrasse - ") and dejaTerrasse == 0:
        print("L'arrete "+str(i)+","+str(j)+" est de la classe \"terrasse\" !")
        terrasses['arretes'].append(data['data'][i]['content']['arretes'][j])
        dejaTerrasse += 1
      if contains(data['data'][i]['content']['arretes'][j]['title'][k]['text'], "AMBULANTE DE PIZZA") and dejaPizza == 0:
        print("L'arrete "+str(i)+","+str(j)+" est de la classe \"camion à pizza\" !")
